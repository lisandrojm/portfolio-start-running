...
export function FloatingLabelInput({ label, ...props }: FloatingLabelInputProps) {
  ...
  return (
    <div className="relative">
      ...
      <input {...props} className="block w-full border-0 px-4.5 pt-5 bg-transparent text-white shadow-sm ring-1 ring-inset ring-white focus:ring-2 focus:ring-inset focus:ring-blue sm:text-sm sm:leading-6" onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} />
    </div>
  );
}
